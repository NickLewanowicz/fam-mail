...
/**
 * Strip HTML tags from markdown text.
 *
 * The message field supports markdown but not raw HTML. This function
 * removes HTML tags while preserving markdown syntax and plain text.
 *
 * This prevents users from injecting raw HTML that might bypass
 * markdown processing or cause unexpected rendering behavior.
 *
 * @param markdown - The markdown text that may contain HTML tags
 * @returns Sanitized markdown with HTML tags removed
 */
export function sanitizeMarkdown(markdown: string): string {
  ...
  // Remove HTML tags using a simple regex-based approach.
  // This is safe for our use case because:
  // 1. The message is markdown, not HTML
  // 2. We're stripping all tags, not trying to preserve any
  // 3. The final HTML output will be generated by marked() and sanitized by DOMPurify
  //
